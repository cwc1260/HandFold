save_dir='../data/ICVL_center_pre0/';
...
for file_idx = 1:length(labels{1})
    ...
    xyz = [ preds{1}(file_idx), preds{2}(file_idx),preds{3}(file_idx);
        preds{4}(file_idx),preds{5}(file_idx),preds{6}(file_idx);
        preds{7}(file_idx),preds{8}(file_idx),preds{9}(file_idx);
        preds{10}(file_idx),preds{11}(file_idx),preds{12}(file_idx);
        preds{13}(file_idx),preds{14}(file_idx),preds{15}(file_idx);
        preds{16}(file_idx),preds{17}(file_idx),preds{18}(file_idx);
        preds{19}(file_idx),preds{20}(file_idx),preds{21}(file_idx);
        preds{22}(file_idx),preds{23}(file_idx),preds{24}(file_idx);
        preds{25}(file_idx),preds{26}(file_idx),preds{27}(file_idx);
        preds{28}(file_idx),preds{29}(file_idx),preds{30}(file_idx);
        preds{31}(file_idx),preds{32}(file_idx),preds{33}(file_idx);
        preds{34}(file_idx),preds{35}(file_idx),preds{36}(file_idx);
        preds{37}(file_idx),preds{38}(file_idx),preds{39}(file_idx);
        preds{40}(file_idx),preds{41}(file_idx),preds{42}(file_idx);
        preds{43}(file_idx),preds{44}(file_idx),preds{45}(file_idx);
        preds{46}(file_idx),preds{47}(file_idx),preds{48}(file_idx)];

    uvd = zeros(size(xyz));
    uvd(:,1) = fFocal_MSRA_ * xyz(:,1) ./ xyz(:,3) + 160;
    uvd(:,2) = 120 - fFocal_MSRA_ * xyz(:,2) ./ xyz(:,3);
    uvd(:,3) = xyz(:,3);


    palm = [uvd(1,1) uvd(1,2) uvd(1,3)];
    ...
    left = max(min(floor(min(uvd(:,1)) - 20), palm_left), 1);
    right = min(max(ceil(max(uvd(:,1)) + 20), palm_right), 320);
    top =  max(min(floor(min(uvd(:,2)) - 20), palm_top), 1);
    bottom = min(max(ceil(max(uvd(:,2)) + 20), palm_bottom), 240);
    back = max(ceil(max(uvd(:,3)) + 20), palm_back);
    front =  min(floor(min(uvd(:,3)) - 20), palm_front);

...
parser.add_argument('--dataset_path', type=str, default = './data/msra_process/',  help='model name for training resume')
...
save_dir = os.path.join(opt.save_root_dir, 'msra_folding', subject_names[opt.test_index])
...
test_data = HandPointDataset(root_path=opt.dataset_path, opt=opt, train = False)
...
for epoch in range(opt.nepoch):
	...
	for i, data in enumerate(tqdm(train_dataloader, 0)):
		...
		points, gt_xyz = rotate_point_cloud_by_angle_flip(points, gt_xyz.view(-1, 21, 3), True)

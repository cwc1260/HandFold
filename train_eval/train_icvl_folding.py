...
from utils import group_points, rotate_point_cloud_by_angle_flip, rotate_point_cloud_by_random
...
parser.add_argument('--test_index', type=int, default = 7, help='test index for cross validation, range: 0~8')
parser.add_argument('--save_root_dir', type=str, default='../results',  help='output folder')
parser.add_argument('--model', type=str, default = '',  help='model name for training resume')
parser.add_argument('--optimizer', type=str, default = '',  help='optimizer name for training resume')
parser.add_argument('--train_path', type=str, default = '../data/ICVL_center_single1/Training',  help='model name for training resume')
...
save_dir = os.path.join(opt.save_root_dir, 'icvl_folding')
...
# 1. Load data
train_data = HandPointDataset(root_path=opt.train_path, opt=opt, train = True)
...
for epoch in range(opt.nepoch):
	...
	for i, data in enumerate(tqdm(train_dataloader, 0)):
		...
		points, gt_xyz = rotate_point_cloud_by_angle_flip(points, gt_xyz.view(-1, opt.JOINT_NUM, 3), True)


		shift = ((torch.rand((points.size(0),3)).cuda() * 20. - 10.) / volume_length).view(-1, 1, 3)
		points[:,:,0:3] = points[:,:,0:3] + shift
		gt_xyz = gt_xyz + shift

		scale = (torch.rand(points.size(0)).cuda() * 0.2 + 0.9).view(-1, 1, 1)
		points = points * scale
		gt_xyz = gt_xyz * scale
